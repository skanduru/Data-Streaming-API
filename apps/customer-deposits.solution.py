...
bankDepositsStreamingDF = bankDepositsRawStreamingDF.selectExpr("cast(key as string) key", "cast(value as string) depositMessage")
...
bankDepositsStreamingDF.withColumn("depositMessage",from_json("depositMessage",depositKafkaMessageSchema))\
        .select(col('depositMessage.*')) \
